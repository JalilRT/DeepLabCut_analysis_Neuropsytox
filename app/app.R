...
TrackOF <- function(input_folder, OF, OF_Rotation, name){
...
pipeline_1 <- function(path, fps, bodypoint, EPM_zoneinfo){
  ...
  zoneinfo <- read.table(EPM_zoneinfo, sep = ",", header = TRUE)
  ...
  Tracking <- EPMAnalysis(Tracking, movement_cutoff = 5, integration_period = 5,points = bodypoint, nosedips = TRUE)
  return(Tracking)
}
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(width = 2,
                 ...
                 materialSwitch(
                   inputId = "additional_csv_switch",
                   label = "Add csv demographics to plot some statistical graphs?",
                   value = FALSE,
                   status = "primary"
                 ),
                 conditionalPanel(
                   condition = "input.additional_csv_switch == true",
                   fileInput("additional_csv", "Select demographics csv:", accept = ".csv")
                 ),
...
server <- function(input, output, session) {
  ...
  output$graph_output <- renderPlotly({
    ...
    if (is.null(input$additional_csv)) {
      ...
      ggplotly(ggplot(Anx_Ind, aes(x = x(), y = y(), fill = z() )) +
                 geom_point() +
                 geom_boxplot() +
                 theme_minimal() +
                 xlab(input$x_var) +
                 ylab(input$y_var) +
                 theme(axis.text.x = element_text(angle = 45, hjust = 1),
                       legend.position = "none") + stat_compare_means(method = "wilcox.test",ref.group = ".all.")) 
    }
  })
  
  
  observeEvent(results(), {
    updateSelectInput(session, "x_var", choices = colnames(results()))
    updateSelectInput(session, "y_var", choices = colnames(results()))
    updateSelectInput(session, "z_var", choices = colnames(results()))
  })
